	...
	def play(self):
		if self.state == 0:
			if self.assigned:
				...
				t2 = sorted([(snaffle, dist(self.pos, snaffle.pos)) for snaffle in snaffles if my_team_id and abs(snaffle.pos[0]-goals[my_team_id][0]) < 400], key=lambda elem:dist(self.pos, elem[0].pos)) # Defence
				t = None
				if t1 and t2:
...
def evaluate():
	...
	if code == CODE_GREEN  or code == CODE_RED:
		for wizard in wizards:
			if wizard.state == 0:
				t = sorted([snaffle for snaffle in snaffles if not snaffle.state and not snaffle.assigned], key= lambda snaffle: (dist(wizard.pos, snaffle.next_pos), (dist(goals[my_team_id^1], wizard.pos))))
				if t :
